...
import { Modal, ripple as dayRipple } from './ui';
import { ChevronIcon } from './icons';
import { MONTH_NAMES, WEEKDAY_HEADERS, parseDateKey, todayKey } from '../lib/dates';
import { colors, radius } from '../theme';

// The month chevrons and "Jump to today" are bare glyphs with no surface of
// their own, so they take Android's borderless ripple; the day cells are
// real rounded buttons and take the bounded one.
const monthRipple = { color: 'rgba(244,239,233,0.16)', borderless: true, radius: 20 };
  ...
  return (
    <Modal open={open} onClose={onClose} title={`${MONTH_NAMES[viewMonth]} ${viewYear}`}>
      <View style={styles.nav}>
        <Pressable onPress={goPrevMonth} android_ripple={monthRipple} hitSlop={10} style={styles.navButton}>
          <ChevronIcon color={colors.paperDim} direction="left" />
        </Pressable>
        <Pressable onPress={() => onSelect(todayDk)} android_ripple={monthRipple} hitSlop={10}>
          <Text style={styles.jumpToday}>Jump to today</Text>
        </Pressable>
        <Pressable onPress={goNextMonth} android_ripple={monthRipple} hitSlop={10} style={styles.navButton}>

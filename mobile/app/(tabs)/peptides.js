...
import { ALL_DAYS, dayShortSummary, formatDateKeyFromDate } from '../../src/lib/dates';
  ...
  return (
    <Card style={styles.card}>
      ...
      <View style={styles.cardFooter}>
        ...
        <Pressable onPress={onToggleExpand} android_ripple={linkRipple} hitSlop={8}>
    ...
    <Modal open={open} onClose={onClose} title="Add reconstituted vial">
      <View style={styles.form}>
        ...
        <Field label="Reconstituted on">
          {/* The web form used <input type="date">; DateField opens the
              system date dialog, which also removes any chance of typing a
              date the parser won't accept. A vial can't have been mixed in
              the future, so today is the ceiling. */}
          <DateField
          ...
          {finished.map((p) => (
            <Card key={p.id} style={styles.finishedCard}>
              <View style={styles.rowBetween}>
                ...
                <Pressable onPress={() => deletePeptide(user.uid, p.id)} android_ripple={linkRipple} hitSlop={8}>

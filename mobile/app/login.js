...
import { Button, Input, ripple } from '../src/components/ui';
...
export default function LoginScreen() {
  ...
  // The two platforms get out of the keyboard's way differently. iOS needs
  // KeyboardAvoidingView to pad the form up. Android doesn't: app.json asks
  // for `softwareKeyboardLayoutMode: "resize"`, so the window itself shrinks
  // and this ScrollView simply gets shorter — layering the view on top of
  // that double-counts the inset and pushes the form off screen. Hence
  // `behavior={undefined}` there rather than a second component tree.
  return (
    <KeyboardAvoidingView
      style={styles.root}
...
const styles = StyleSheet.create({
  ...
  submit: { marginTop: 8 },
  link: { paddingVertical: 10, borderRadius: 8, overflow: 'hidden' },

...
// Material's touch response is a ripple that spreads from the finger, not
// the whole-view dim iOS uses. Everything tappable here is a rounded
// surface, so the ripple is bounded — which needs `overflow: 'hidden'` on
// the same view, or it paints square corners over the radius.
...
// the native shape of the same thing, and onRequestClose is what wires the
// Android back button and the back gesture to it — without that prop, back
// closes the whole app instead of the sheet.
export function Modal({ open, onClose, title, children }) {
  ...
  return (
    <RNModal
      ...
      // The app is drawn edge-to-edge, so the sheet's dark scrim has to
      // reach under both system bars too — otherwise they stay lit at the
      // app's own background colour while everything behind them dims.
